...
hr_zone_select_t get_hr_zone_select(void){
	if( user_metrics_info.hr_zone_calc_select > hr_zone_select_RHR ){
		user_metrics_info.hr_zone_calc_select =  hr_zone_select_RHR;
	}
...
float  get_hr_SPIVI_info( zonesEnum zone ){
    return (float)((((float)MAX_HR - (float)obter_idade())*(float)get_SPIVI_percentage_threshold(zone))/(float)100);
}
...
float get_hr_rhr_info( zonesEnum zone ){
    return (float)((((float)max_hr_get() - (float)get_resting_heart_rate_value())*(float)get_rhr_percentage_threshold(zone))/(float)100+(float)get_resting_heart_rate_value());
}
/******************************************************************************/
/******************************************************************************/
...
void zones_info_init( void )
{
	...
	if(user_metrics_info.resting_hr/*get_hr_zone_select() == hr_zone_select_RHR */)
	{
...
//		zonesEnum temp;
		...
		perc_hr_max=(BPM*100)/max_hr_get();
